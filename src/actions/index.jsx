...
function fetchRelatedMoviesFail(error) {
	return {
		type: FETCH_RELATED_MOVIES_FAILURE,
		error,
	};
}
export function fetchRelatedMoviesList(id) {
	const url_relatedMovies = URL_DETAIL + id + URL_RELATED_MOVIES + API_KEY;
	console.log(url_relatedMovies);
	console.log(id);
	return function (dispatch) {
		dispatch(fetchRelatedMovies());
		return fetch(url_relatedMovies)
			.then((response) => response.json())
